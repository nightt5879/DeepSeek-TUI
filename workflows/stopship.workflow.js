      ...
      "sequence": {
        ...
        "children": [
          ...
          },
          {
            "agent": {
              "id": "release-receipt",
              "prompt": "Use only the promoted verification_report handoff to produce the final acceptance receipt for the Fleet/Workflow/Lane/Runtime contract. Tools are intentionally unavailable because promoted handoffs are the evidence boundary; do not request source reads or gather new evidence. You have at most three model responses, but the first response must return APPROVE or BLOCK with no tool calls. After that verdict, include a `FINAL RECEIPT` section carrying forward declared Fleet role and resolved profile evidence, every observed gate state, the required terminal workflow status, the tmux Lane process-exit reconciliation owner, and their concrete source owners. A bare verdict is invalid. The first non-empty line of your response must be exactly APPROVE or exactly BLOCK. Do not put any words before that verdict: no confirmation, summary, heading, or phrase such as `Here is the verdict`. Use APPROVE only when that receipt can be produced from the handoff; otherwise use BLOCK and name the closure blocker. Never claim that source inspection substitutes for the host's live Lane log; the host validates the actual process-exit receipt after this role completes. Do not edit, publish, close issues, run shell commands, or mutate the workspace.",
              "agent_type": "general",
              "role": "release_lead",
              "mode": "read_only",
              "permissions": { "deny_all_tools": true },
