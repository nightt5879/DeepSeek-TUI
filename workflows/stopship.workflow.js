        ...
        "children": [
          {
            "agent": {
              "id": "scout-runtime",
              "prompt": "Verify the release-orchestration owners using only the five files in File scope. You have at most six model responses and must reserve the verdict. Response 1 must make exactly one `grep_files` call with `path` set to `.` and this high-signal alternation pattern: `stopship-release-acceptance|name = \"stopship\"|load_named_fleet|start_lane|resolve_workflow_agent|record_task_started|WorkflowUiEventKind::GateUpdated|WorkflowUiEventKind::RunCompleted|stopship_acceptance_fixture_emits_role_gate_and_terminal_receipts`. Set at most 80 results and 2 context lines. Do not add generic field names such as `resolved_profile` to the pattern. Do not call `grep_files` more than once and do not call `read_file`, `list_dir`, `file_search`, or any other tool. Response 2 must return the verdict with no tool calls; any later reserved response must do the same instead of gathering more evidence. Treat an exact match naming a call site, typed event constructor, or test assertion in a scoped file as source-owner evidence. If the single grep does not cover every required item, return BLOCK. The first non-empty line of your response must be exactly APPROVE or exactly BLOCK. Do not put any words before that verdict: no confirmation, summary, heading, or phrase such as `Here is the verdict`. After the verdict, include a `SOURCE EVIDENCE` section with concise `path: symbol` evidence for the stopship alias, named Fleet loading, role-to-profile resolution, tmux Lane launch, typed task_started, gate_updated, and terminal run_completed receipts. A bare verdict is invalid. Do not edit files, create branches, run shell commands, access GitHub, or infer success where source evidence is absent.",
              "agent_type": "explore",
              "role": "scout",
              "mode": "read_only",
              "file_scope": [
