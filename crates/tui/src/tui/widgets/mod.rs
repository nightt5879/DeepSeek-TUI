    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if input_text.is_empty() {
            ...
            if input_rows_budget > 1 {
                let (placeholder, is_suggestion) = composer_empty_hint(self.app);
                let style = if is_suggestion {
        ...
        let visual_rows = if input_text.is_empty() {
            let hint = composer_empty_hint_text(self.app);
            empty_composer_visual_rows(Some(hint.as_ref()), content_width, input_rows_budget)
        } else {
            input_lines.len()
        };
...
fn composer_empty_hint(app: &App) -> (Cow<'_, str>, bool) {
    if let Some(ref suggestion) = app.prompt_suggestion
        && !app.is_history_search_active()
    {
        (Cow::Borrowed(suggestion.as_str()), true)
    } else {
        (composer_empty_placeholder_text(app), false)
    }
}

pub(crate) fn composer_empty_hint_text(app: &App) -> Cow<'_, str> {
    composer_empty_hint(app).0
}

fn composer_empty_placeholder_text(app: &App) -> Cow<'static, str> {
